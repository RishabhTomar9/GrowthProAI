...
// ✅ Fallback logic for backend URL
const API_BASE =
  import.meta.env.VITE_API_BASE ||
  (import.meta.env.DEV
    ? "http://localhost:5000"
    : "https://growthproai-mfra.onrender.com"); // Replace with your actual Render backend if different
...
const Dashboard = () => {
  ...
  const regenerateHeadline = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorText = await response.text();
        console.error("❌ Regenerate response:", errorText);
        throw new Error("Backend error");
      }
